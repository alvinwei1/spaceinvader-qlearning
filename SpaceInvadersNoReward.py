...
nonflat_min = [[[0,0,0]] * 160] * 210
MIN_VALUES = sum(sum(nonflat_min, []), [])
nonflat_max = [[[255,255,255]] * 160] * 210
...
def train(agent, env, history, num_episodes=NUM_EPISODES):
  for i in xrange(NUM_EPISODES):
    ...
    # print(len(obs))
    # if type(obs) is list:
    obs_flat = list(flatten(obs.tolist()))
    print('obs len:', len(obs))
    ...
    for t in xrange(MAX_STEPS):
      ...
      obsersvation_flat = list(flatten(observation.tolist()))
      next_state = discretize(obsersvation_flat)
      # else:
...
if RECORD:
  env = wrappers.Monitor(env, '/tmp/spaceinvaders-experiment-1', force=True)
